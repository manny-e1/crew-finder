...
import { imageUploadVisibilityAtom } from '../atoms/changeElementVIsibilityAtoms';
...
function ImageUpload() {
  const [avat, setAvat] = useState<string | null>(null);
  const [file, setFile] = useState<File>();
  // const dispatch = useDispatch();

  const imageHandler = (e: ChangeEvent) => {
  ...
  // const { currentUser } = useSelector((state) => state.userUpdate);
  const [currentUser] = useAtom(currentUserAtom);
  const [_, setImageUploadVisibility] = useAtom(imageUploadVisibilityAtom);
  ...
  const submitHandler = (e: MouseEvent<HTMLButtonElement>) => {
    const formData = new FormData();
    formData.append('showcasePics', file as Blob);
  ...
  return (
    <div className="absolute inset-x-6 bg-white md:inset-x-10 lg:inset-x-1/3">
      <div className="flex justify-between border-b p-5 ">
        <h2>Edit Photo</h2>
        <div onClick={hideImageUpload} className="cursor-pointer text-xl">
          X
        </div>
      </div>
      <div className="flex h-80 flex-col items-center justify-center space-y-5 border-b">
        <div className="">
          <img
            src={
              avat ??
            ...
            className="flex h-40 w-40 items-center justify-center rounded-full border-2 border-blue-500"
          />
        </div>
        <form>

import { MapPinIcon, PencilIcon } from '@heroicons/react/24/solid';
import { useMutation, useQuery } from '@tanstack/react-query';
import { AxiosError } from 'axios';
import { useAtom } from 'jotai';
import { MouseEvent, useEffect, useState } from 'react';
import { useSelector, useDispatch } from 'react-redux';
import { useParams } from 'react-router-dom';
import {
  editButtonVisibilityAtom,
  imageUploadVisibilityAtom,
} from '../atoms/changeElementVIsibilityAtoms';
import { currentUserAtom } from '../atoms/localStorageAtoms';
import CatagoryButton from '../components/CategoryButton';
import EditProfile from '../components/EditProfile';
import Header from '../components/Header';
import ImageUpload from '../components/ImageUpload';
import {
  endorseUser,
  getGivenEndorsements,
  getReceivedEndorsements,
  IEndorsement,
} from '../services/endorsementService';
import { getUserDetail, IUser } from '../services/userService';
// import {
//   endorseUser,
//   getGivenEndorsements,
//   getReceivedEndorsements,
// } from '../store/endorsement/api.endorsement';
import {
  hideEdit,
  hideImageUpload,
  unhideDivImageUpload,
  unhideEdit,
} from '../store/ui/hideDiv';
import { getUserDetails, updateUserProfile } from '../store/user/api.user';
import { Role } from '../util/enums';
import { capitalizeFirstLetter } from '../util/firstLetterCapitalizer';

type EndorsementError = AxiosError<{ message: string } | any>;

function Profile() {
  const { id } = useParams();

  const [currentUser] = useAtom(currentUserAtom);
  const [editButtonVisibility, setEditButtonVisibility] = useAtom(
    editButtonVisibilityAtom
  );
  const [imageUploadVisibility, setImageUploadVisibility] = useAtom(
    imageUploadVisibilityAtom
  );

  const {
    isLoading,
    data: user,
    error,
  } = useQuery<IUser, EndorsementError>(['userDetail', id], () =>
    getUserDetail(id ?? '')
  );
  const {
    isLoading: givenEndorsementsLoading,
    data: givenEndorsements,
    error: givenEndorsementsError,
    refetch,
  } = useQuery<IEndorsement[], EndorsementError>(
    ['givenEndorsements', id],
    getGivenEndorsements,
    {
      enabled: user?._id !== currentUser?.id && user?.role === Role.proDirector,
    }
  );
  const {
    isLoading: receivedEndorsementsLoading,
    data: receivedEndorsements,
    error: receivedEndorsementsError,
  } = useQuery<IEndorsement[], EndorsementError>(
    ['receivedEndorsements', id],
    () => getReceivedEndorsements(id ?? ''),
    {
      enabled: id != undefined && user?.role !== Role.proDirector,
    }
  );
  const {
    isLoading: endorseUserLoading,
    error: endorseUserError,
    mutate,
  } = useMutation<any, EndorsementError, { endorseeId: string }>(endorseUser, {
    onSuccess: (_) => {
      refetch();
    },
  });

  // const { currentUser } = useSelector((state) => state.userLogin);
  // const { loading, user, error } = useSelector((state) => state.userDetail);
  // const { givenEndorsements } = useSelector((state) => state.givenEndorsements);
  // const { receivedEndorsements } = useSelector(
  //   (state) => state.receivedEndorsements
  // );
  useEffect(() => {
    // dispatch(getUserDetails(id));
    // dispatch(hideImageUpload());
    // dispatch(hideEdit());
    setImageUploadVisibility({ display: 'hidden' });
    setEditButtonVisibility({ display: 'hidden' });
  }, []);

  const endorseHandler = (e: MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    mutate({ endorseeId: id! });
  };

  useEffect(() => {
    // if (user?._id !== currentUser._id && user?.role !== 'PRO_DIRECTOR')
    //   dispatch(getGivenEndorsements());
  }, []);
  useEffect(() => {
    // if (user?.role !== 'PRO_DIRECTOR') dispatch(getReceivedEndorsements(id));
  }, []);
  // console.log(receivedEndorsements);
  const unhideImageUpload = (e: MouseEvent<HTMLDivElement>) => {
    e.preventDefault();
    setImageUploadVisibility({ display: 'flex' });
    // dispatch(unhideDivImageUpload());
  };
  const unhideEditButton = (e: MouseEvent<HTMLDivElement>) => {
    e.preventDefault();
    setEditButtonVisibility({ display: 'flex' });
    // dispatch(unhideDivImageUpload());
  };

  // const { hidden } = useSelector((state) => state.hideImageUpload);
  // const hiiideEdit = useSelector((state) => state.hideEdit);

  if (isLoading)
    return <div className="flex w-full justify-center">Loading...</div>;
  if (error)
    return (
      <p className="flex justify-center bg-red-500">
        {error?.response?.data?.['message']}
      </p>
    );

  return (
    <div>
      <div className="relative mx-auto mt-5 border lg:max-w-5xl">
        <div
          className={
            'items-center justify-center bg-transparent ' +
            imageUploadVisibility.display +
            ' absolute inset-x-0 inset-y-0 z-10'
          }
        >
          <ImageUpload />
        </div>
        <div
          className={
            'items-center justify-center bg-transparent ' +
            editButtonVisibility.display +
            ' absolute inset-x-0 inset-y-0 z-10'
          }
        >
          <EditProfile />
        </div>
        <div className="relative flex w-full items-center border-b-2 py-4 px-5 md:mx-10 md:my-5">
          <div className="relative">
            <img
              className="h-20 w-20 rounded-full"
              alt=""
              src={
                user?.avatar ??
                'data:image/jpeg;base64,'
              }
            />
            {user?._id === currentUser?.id ?? (
              <div
                className="absolute top-1 flex h-6 w-6 items-center justify-center rounded-full bg-white"
                onClick={unhideImageUpload}
              >
                <PencilIcon className="h-4  " />
              </div>
            )}
          </div>
          <div className="items-center pl-4  sm:pl-8">
            <h1 className="text-2xl">{user?.fullName}</h1>
            <p className="flex items-center text-sm">
              <MapPinIcon className="h-4 pr-1 text-blue-600" />{' '}
              {user?.address?.region},{' '}
              {user?.address?.country === 'ET'
                ? 'Ethiopia'
                : user?.address?.country}
            </p>
          </div>
        </div>{' '}
        <div className="space-y-2 border-b-2 py-3 px-5">
          {user?._id === currentUser?.id && (
            <div className=" flex items-center space-x-6 ">
              <h2 className="text-xl">Edit Profile</h2>
              <div
                className="flex h-8 w-8 items-center justify-center rounded-full border-2"
                onClick={unhideEditButton}
              >
                <PencilIcon className="h-5" />
              </div>
            </div>
          )}
          <div className="space-y-4">
            <h2 className="text-xl font-bold">
              {capitalizeFirstLetter(user?.talent!)}
            </h2>
            <p className="text-sm font-light md:text-base">
              {user?.bio ?? user?._id === currentUser?.id
                ? 'Your bio is empty'
                : ''}
            </p>
          </div>
        </div>
        <div className="border-b-2 py-3 px-5">
          <h2 className="font-bold">Other Talents</h2>
          <div className="mt-1">
            {user?.otherTalents?.map((talent) => (
              <CatagoryButton text={talent} />
            ))}
          </div>
        </div>
        <div className="">
          <h2 className="px-5 pt-3 text-xl  font-bold">Personal Information</h2>
          <div className="flex justify-between px-5 pb-2">
            <div className="flex items-center space-x-4">
              <h3>Email:</h3>
              <p className="font-light">{user?.email}</p>
            </div>
          </div>
          <div className="px-5 pb-2 ">
            <div className="flex justify-between">
              <h3>Location</h3>
            </div>
            <div className="flex space-x-4 pl-3 text-sm">
              <h4>Country:</h4>
              <p className="font-light">{user?.address?.country}</p>
            </div>
            <div className="flex space-x-4 pl-3 text-sm">
              <h4>Region:</h4>
              <p className="font-light">{user?.address?.region}</p>
            </div>
          </div>
          <div className="flex justify-between border-b-2 px-5 pb-2">
            <div className="flex items-center space-x-4">
              <h3>Gender:</h3>
              <p className="font-light">
                {capitalizeFirstLetter(user?.gender!)}
              </p>
            </div>
          </div>
        </div>
        <div className="flex justify-between border-b-2 py-3 px-5">
          <div className="">
            <h2 className="font-bold">Endorsements</h2>
            {receivedEndorsements?.length === 0 ? (
              <p className="text-sm font-light">
                Endorsements from past clients
              </p>
            ) : (
              receivedEndorsements?.map((received) => (
                <p>{(received?.endorser as IUser).fullName}</p>
              ))
            )}
          </div>
        </div>
        {/* <div className="py-3 px-5 border-b-2 flex justify-between">
          <div className="">
            <h2 className="font-bold">Certifications</h2>
            <p className="text-sm font-light">
              Add certifications to highlight your best skills
            </p>
          </div>
          <button className="border-2 h-8 w-8 flex justify-center items-center rounded-full">
            <p className="text-2xl font-light">+</p>
          </button>
        </div> */}
      </div>
      {givenEndorsements?.some(
        (given) => (given.endorsee as IUser)._id === id
      ) ? (
        <button
          className="mx-auto flex rounded-full bg-red-500 py-2 px-4"
          // onClick={() => dispatch(endorseDele)}
        >
          remove Endorse
        </button>
      ) : (
        <button
          className="mx-auto flex rounded-full bg-green-500 py-2 px-4"
          onClick={endorseHandler}
        >
          Endorse
        </button>
      )}
    </div>
  );
}

export default Profile;
